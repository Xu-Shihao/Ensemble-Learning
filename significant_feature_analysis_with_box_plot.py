...
def main():
    ...
    save_directory = './distribution_single'
    ...
    for FeaSet_id in range(len(FeatureSetName_pool)):
        if FeatureSetName_pool[FeaSet_id] in ['Diction', 'LIWC', 'OpenSmile', 'DisVoice', 'Conver', 'Movement',
                                              'Affectiva', 'Opsis', 'OpenFace','corr']:
            features_name = []
            features = []
    ...
    print('task_abbreviation: ', ''.join(sorted(task_abbreviation)))
    ...
    for FeaSet_id in range(len(FeatureSetName_pool_save)):
        ...
        for i in range(len(rank)):
            fn_pvalue.append(
                [LoadFeatureNames_pool_save[FeaSet_id][rank[i]], Features_pValue_pool_save[FeaSet_id][rank[i]]])
...
def plot_his(features_frame, label, abbr, save_directory):
    # select feature set
    for j in range(0, len(features_frame.columns)):

        feature_name = features_frame.columns[j]

        fig, axes = plt.subplots(nrows=1, ncols=1, figsize=(4, 4))

        df = features_frame[feature_name]

        if len(set(label)) == 3:

            filter_col_A = [location for location, label_ in enumerate(label) if label_ == 0]
            sampleA = df.loc[df.index[filter_col_A]].values.tolist()

            filter_col_B = [location for location, label_ in enumerate(label) if label_ == 1]
            sampleB = df.loc[df.index[filter_col_B]].values.tolist()

            filter_col_C = [location for location, label_ in enumerate(label) if label_ == 2]
            sampleC = df.loc[df.index[filter_col_C]].values.tolist()

            all_data = [sampleA, sampleB, sampleC]
            abbr_list = [abbr[0], abbr[1], abbr[2]]
        ...
        plt.setp(axes, xticks=[y + 1 for y in range(len(all_data))],
                 xticklabels=abbr_list)
        fig.savefig(save_directory + '/' + abbr + '_' + feature_name + '.png', dpi=fig.dpi)
...
def plot_his_2Class(p_value_list_path, entire_features_path, abbr):
    ...
    for j in range(0, int(len(salient_features_list.columns) / 2)):
        ...
        for i in range(plot_fig_num):
            ...
            fig.savefig(
                save_directory + '/' + abbr + '_' + salient_features_list[j * 2][0] + '_' + feature_name + '.png',
                dpi=fig.dpi)
